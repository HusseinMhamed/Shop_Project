...
function drow() {
    ...
    products.forEach(product => {
        ...
        for(key in addednewItem){
            if(key==product.name){
                chose=true
                container.innerHTML += generateCartItemHTML(product);
            }
        }
        if(chose==false){
    ...
    return `
        <div class="card col-lg-3 col-md-4 col-sm-6 m-3" style="width: 18rem;">
            ...
            <div class="card-body">
                ...
                <a class="btn btn-danger" onclick="removeitem('${product.name}')">Remove from cart</a>
                <a style="float: right; font-size: larger;" id="heart"></a>
            </div>
        </div>`;
}
    ...
    return `
        <div class="card col-lg-3 col-md-4 col-sm-6 m-3" style="width: 18rem;">
            ...
            <div class="card-body">
                <h5 class="card-title">${product.name}</h5>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                <a class="btn btn-primary" onclick="addtocard(${product.id})">Add to cart</a>
                <a style="float: right; font-size: larger;" id="heart"></a>
            </div>
        </div>`;
}
...
function favorit(){
    fav=document.getElementById("heart")
    let addednewItem =  JSON.parse(localStorage.getItem(storeduser + "_cart")) || []
    for(var key in addednewItem){
        if(addednewItem[key].favorite){
           fav.innerHTML=`<i class="fas fa-heart"></i>`
        }
        else{
            fav.innerHTML=`<i class="far fa-heart"></i>`
        }
        // console.log(addednewItem[key].favorite)
    }
    // console.log(addednewItem)
